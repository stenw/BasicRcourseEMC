...
Try to keep your work in a single script file. As you create each object, do not just print it: also inspect it and ask yourself what kind of object it is.
...
::: callout-tip
As you compare your work with the solutions, keep using inspection functions such as `class(...)`, `mode(...)`, `length(...)`, `dim(...)` and `str(...)`. In this chapter, those checks are often just as important as the code that creates the object. The most frustrating bugs in R happen when an object looks right but has the wrong 'shape' or `class` for a function. Using these checks as you go turns a guessing game into a precise audit of why your code works (or why it doesn't).
:::
...
After looking at the first and last rows, ask yourself: which variables seem numeric, and which look categorical?
...
After printing them, check whether both are stored in the same way.
...
Once you have made these vectors, compare them. Which are numeric, which are character, and how can you tell?
...
After creating each list, inspect it with `str(...)`. Which parts are vectors, and how does `R` show that they are inside a list?
...
- check that you know which inspection functions are most useful for vectors, matrices, `data.frame`s, lists and arrays
