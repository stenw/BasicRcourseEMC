...
- extract values from matrices and arrays
- use logical conditions to create meaningful subsets
- combine subsetting with summary functions to answer simple data questions

This answer file follows the same structure as the practical. Use it to compare your own code and to check whether you understand what kind of object each subsetting operation returns.
...
::: callout-tip
When you compare your code with the solutions, pay attention not only to the values returned but also to the class of the result.
:::
...
This task is mainly about becoming comfortable with the row-and-column structure of a `data.frame`.
:::
...
This is one of the most important comparisons in the chapter: similar-looking syntax can return either a `data.frame` or a vector.
:::
...
Notice that selecting a single element gives one value back, whereas selecting a row or column returns multiple values.
:::

::: callout-note
## Milestone

At this point, you should feel comfortable with the idea that very similar subsetting expressions can return different kinds of objects.

In particular, make sure you can explain the difference between:

- selecting a column as a `data.frame`
- selecting a column as a vector
- selecting one value from a matrix versus a whole row or column
:::
...
This is a more advanced form of indexing, but it shows that matrices can themselves be used to describe positions.
:::
...
This task is useful because it combines object construction with meaningful selection from a multi-dimensional structure.
:::
...
This is where subsetting starts to look like real data analysis: we are selecting groups defined by study variables.
:::
...
Several of these tasks are logically similar, but they answer different questions. That is exactly why subsetting matters in practice.
:::
...
This section shows the typical pattern: first select the relevant observations, then apply a summary function.
:::

::: callout-note
## Milestone

You are now using subsetting for its real purpose: answering questions about specific groups in the data.
:::
...
- identify one example where subsetting returned a vector
- identify one example where subsetting returned a `data.frame`
- check whether you can explain why they differ
- If you made a script, remember to rerun the script from top to bottom and see whether everything still works
:::
