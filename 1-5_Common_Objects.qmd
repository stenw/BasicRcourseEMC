...
Elementary objects can be combined to form more complex objects. This leads to several types of containers, such as `lists` and `data.frames`.
...
Functions are special objects that can do things with other objects, for example the `print` function displays the contents of an object in the console. Functions are the topic of another chapter, but because we obviously want to do something with the various objects we will see, we cannot avoid them altogether.

This is one of those chapters that saves people a lot of frustration later. In `R`, you are almost never working with loose values for long. Very quickly, everything becomes an object of some kind. If you can recognise what sort of object you are looking at, a lot of code suddenly becomes easier to read. If you cannot, even simple errors start to feel strange.

So this chapter is really about learning to look at data structures with the right questions in mind. We will work through the common object types, talk a little about `mode` and `class`, and get used to inspecting unfamiliar objects instead of guessing what they are.

::: callout-tip
### A useful habit

Whenever you encounter an unfamiliar object, start by asking a few simple questions:
...
You do not always need all of these, but together they often tell you almost everything you need to know.
:::

## A quick comparison

Before going into the details, it helps to have a rough mental map:

| Object type | Can mix data types? | Has dimensions? | Typical use |
|---|---|---|---|
| vector | No | Usually one dimension | One variable or one sequence of values |
| matrix | No | Two dimensions | Numeric or character grid with rows and columns |
| array | No | Two or more dimensions | Higher-dimensional data |
| list | Yes | Not necessarily rectangular | Mixed results, nested structures |
| `data.frame` | Yes, by column | Two dimensions | Most rectangular data sets |

This table is only a first orientation. The sections below provide more detail and examples.
...
Numbers are used for measurements, counts, scores, times and many other quantities. In statistical work, numeric variables are extremely common.
...
Text values are stored as `character` data. These are useful for names, labels or descriptions.
...
In practice, you can think of a vector as "one variable" or "one ordered collection of values". For example, the ages of all patients in a study may be stored as one numeric vector.
...
The second element of the resulting vector is now also of type `character`. In general, it is better not to trust this implicit conversion. Instead, do it explicitly—in this case by using the function `as.character()`.
...
::: callout-note
### What to remember about vectors

Vectors are the foundation of many other structures in `R`. If you understand vectors well, later topics such as subsetting, data frames and functions become much easier.
:::
...
Matrices are useful when your data truly forms a rectangular grid and all values are of the same type. Many mathematical functions in `R` expect matrices.
...
Lists are one of the most flexible object types in `R`. They are often used when different pieces of information belong together, but do not naturally fit in a single rectangular table.
...
For most students, the `data.frame` will become the most important structure in this course. A typical data set has one row per patient and one column per variable, which fits perfectly with the `data.frame` idea.
...
If you are unsure whether a variable should be a `character` or a `factor`, a useful question is this: are these free text labels, or are they categories with a limited set of allowed values?
...
1. Which object type would you normally use for a single variable such as patient age?
2. What is the key difference between a matrix and a `data.frame`?
3. Why might a statistical model return a `list` rather than a vector?
4. Which functions would you use first to inspect an unfamiliar object?
